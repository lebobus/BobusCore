...
public enum Kits implements Listener {
    ...
    if(getKit(p).equals(PvP)) {
    	me.lebobus.bobuscore.kitpvp.kits.PvP.giveKitPvP(p);
    	p.sendMessage(kitReceived);
    }
    
    if(getKit(p).equals(Archer)) {
    	me.lebobus.bobuscore.kitpvp.kits.Archer.giveKitArcher(p);
    	p.sendMessage(kitReceived);
    }
    
    if(getKit(p).equals(Fireman)) {
    	me.lebobus.bobuscore.kitpvp.kits.Fireman.giveKitFireman(p);
    	p.sendMessage(kitReceived);
    }
    
    if(getKit(p).equals(Vampire)) {
    	me.lebobus.bobuscore.kitpvp.kits.Vampire.giveKitVampire(p);
    	p.sendMessage(kitReceived);
    }
